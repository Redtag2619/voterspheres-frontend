...
function getToken() {
  ...
  for (const key of directKeys) {
    ...
    const localValue = cleanToken(localStorage.getItem(key));

    if (localValue) return localValue;

 

    const sessionValue = cleanToken(sessionStorage.getItem(key));
  ...
  for (const key of objectKeys) {
    ...
    const localToken = extractTokenFromJson(localStorage.getItem(key));

    if (localToken) return localToken;

 

    const sessionToken = extractTokenFromJson(sessionStorage.getItem(key));
...
function readWorkspaceIdFromObject(value) {
  ...
  try {
    ...
    const workspaceId = Number(
      ...
      parsed?.workspace_id ||
        ...
        parsed?.activeWorkspace?.id ||

        parsed?.user?.workspace_id ||

        parsed?.user?.workspaceId
...
export function getPoliticalFabricWorkspaceId(fallback = 1) {
  ...
  for (const key of directKeys) {
    ...
    const localValue = Number(localStorage.getItem(key));
  ...
  for (const key of objectKeys) {
    ...
    const localValue = readWorkspaceIdFromObject(localStorage.getItem(key));

    if (localValue) return localValue;

 

    const sessionValue = readWorkspaceIdFromObject(sessionStorage.getItem(key));
...
function normalizePoliticalFabricPayload(rawPayload) {
  ...
  const scan =
    ...
    payload?.scan && typeof payload.scan === "object"
...
export async function fetchPoliticalFabricHealth(
  ...
  workspaceId = getPoliticalFabricWorkspaceId()
...
export async function fetchPoliticalFabricOverview(
  ...
  return normalizePoliticalFabricPayload(payload);

}

 

export async function runPoliticalFabricScan(payload = {}) {
  ...
  const response = await request(
...

    {
      ...
      body: JSON.stringify({
        ...
        include_live_sources:

          payload.include_live_sources !== false,

        refresh_live_sources:

          payload.refresh_live_sources !== false,

        include_news:

          payload.include_news !== false,

        include_fec:

          payload.include_fec !== false,

        include_polling:

          payload.include_polling !== false,

        include_legislation:

          payload.include_legislation !== false,

        include_election_administration:

          payload.include_election_administration !== false,

        include_weather_risk:

          payload.include_weather_risk !== false,
  ...
  return normalizePoliticalFabricPayload(response);

}

 

export async function createPoliticalFabricBrief(payload = {}) {
...
export async function savePoliticalFabricWatchlist(payload = {}) {
...
export async function runPoliticalFabricScenario(payload = {}) {
