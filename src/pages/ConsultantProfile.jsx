...
function normalizeRiskFlag(flag, index) {
  ...
  if (typeof flag === "string" || typeof flag === "number") {
    ...
    const label = String(flag).trim() || `Risk flag ${index + 1}`;

    return {

      id: `risk-flag-${index}-${label}`,
  ...
  const detail =
    ...
    (flag.message && flag.message !== label ? flag.message : "");

 

  return {

    id: `${flag.id || flag.code || String(label)}-${index}`,
...
function RiskFlags({ flags = [] }) {
  ...
  return (
    ...
    <SectionCard
      ...
      title="Consultant Risk Flags"

      subtitle="Disclosure, concentration, overlap, exposure, and network conditions requiring review."

      right={

        <Badge tone={rows.length ? "warning" : "active"}>

          {rows.length ? `${rows.length} flagged` : "No active flags"}
    ...
    >
      ...
      {rows.length ? (
        ...
        <div className="vs-stack">
          ...
          {rows.slice(0, 12).map((flag) => (
            ...
            >
              ...
              <Badge tone={riskTone(flag.severity)}>

                {safeText(flag.severity, "Monitor")}
        ...
        <EmptyState text="No consultant risk flags are active for this profile." />
