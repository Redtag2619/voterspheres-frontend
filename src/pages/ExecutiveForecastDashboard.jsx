...
function itemDetail(item = {}) {
  return item.detail || [
   return item.forecast_type || item.coalition_type || item.relationship_type || "link";,
    item.state,
...
function buildBrief({ predictions, opportunities, risks, momentum, coalitions }) {
  ...
  if (opportunities[0]) lines.push(`${itemEntityName(opportunities[0]) || itemTitle(opportunities[0])} is the strongest opportunity signal with opportunity ${fmtScore(opportunities[0].opportunity_score)}.`);
  if (risks[0]) lines.push(`${itemEntityName(risks[0]) || itemTitle(risks[0])} carries the highest forecast risk at ${fmtScore(risks[0].risk_score)}.`);
  if (momentum[0]) lines.push(`${itemEntityName(momentum[0]) || itemTitle(momentum[0])} has notable momentum at ${fmtScore(momentum[0].momentum_score)}.`);
  ...
  return (
    ...
    >
      ...
      <div className="forecast-brief-grid">
        ...
        <BriefCard title="Avg Confidence" value={fmtScore(summary.avgConfidence)} detail="Average confidence across visible forecast predictions." tone="accent" />
      </div>
