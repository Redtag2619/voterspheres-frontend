...
function unwrapPayload(response) {
  ...
  if (
    ...
    (Array.isArray(payload.data.results) || payload.data.summary)
...
async function loadDonors(params) {
  ...
  if (typeof api.donorNetwork === "function") {
    ...
    const response = await api.donorNetwork(params);
...
function normalizeNetworkData(response) {
  ...
  return {
    ...
    stateBreakdown: Array.isArray(payload.stateBreakdown)

      ? payload.stateBreakdown

      : [],

    committeeBreakdown: Array.isArray(payload.committeeBreakdown)

      ? payload.committeeBreakdown
  ...
  useEffect(() => {
    ...
    async function loadDonorNetwork() {
      ...
      try {
        ...
        const response = await loadDonors(params);

        const data = normalizeNetworkData(response);
      ...
      >
        ...
        <div className="donor-command-toolbar">
          ...
          <input
            ...
            id="donor-network-search"

            name="donor_network_search"

            aria-label="Search donor network"
          ...
          <select id="donor-network-cycle" name="donor_network_cycle" aria-label="Election cycle" className="vs-input" value={cycle} onChange={(event) => setCycle(event.target.value)}>
          ...
          <select
            ...
            id="donor-network-state"

            name="donor_network_state"

            aria-label="Donor state"
      ...
      <div className="vs-grid-2" style={{ marginTop: 18 }}>
        ...
        {filteredResults?.[0] ? (
          ...
          <PoliticalGraphContextPanel
            ...
            entityId={filteredResults[0].id || filteredResults[0].donor_id}

            entityName={filteredResults[0].donor_name || filteredResults[0].name}

            state={filteredResults[0].state || filteredResults[0].state_code}
