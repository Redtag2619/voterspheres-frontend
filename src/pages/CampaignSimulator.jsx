  ...
  scenarios: [
    {
      title: "Base Case",
      probability: "44%",
      outcome: "Stable suburban gains and neutral media conditions.",
      status: "Most Likely"
    },
    {
      title: "Narrative Shock",
      probability: "18%",
      outcome: "Negative media cycle compresses margins.",
      status: "Risk"
    }
  ],
  board: [
    {
      ...
      upside: "61%",
      downside: "47%",
      trigger: "Women suburban turnout"
    },
    {
      race: "GA Senate",
      base: "57%",
  ...
  notes: [
    {
      ...
      note: "Affordability discipline plus suburban turnout remains the cleanest route."
    },
    {
      title: "Most fragile assumption",
      note: "Current model is most exposed to local press instability."
    }
  ]
};
...
const CampaignSimulator = () => {
  const fetcher = useCallback(() => platformApi.simulator(), []);
  ...
  return (
    ...
    >
      <Panel
        title="Scenario Deck"
        subtitle="Most important modeled pathways across the next cycle"
      >
        {loading ? (
          <LoadingState />
        ) : error ? (
              ...
              <div key={item.title} className="vs-sim-scenario-item">
                <div className="vs-sim-scenario-top">
                  <div>
                    ...
                    <div className="vs-sim-scenario-probability">
                      {item.probability}
                    </div>
                  </div>
                  <div
                    className={`vs-sim-scenario-status ${statusClass(
                      item.status
                    )}`}
                  >
                    {item.status}
      ...
      <Panel
        title="Battleground Simulation Board"
        subtitle="Base, upside, and downside ranges by priority race"
      >
        {loading ? (
          <LoadingState />
        ) : error ? (
      ...
      <Panel
        title="AI Simulation Notes"
        subtitle="Highest-signal interpretation from current simulation runs"
        large
      >
        {loading ? (
          <LoadingState />
