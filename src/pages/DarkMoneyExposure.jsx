...
export default function DarkMoneyExposure() {
  ...
  const { setFilters } = useExecutiveFilters();
    ...
    >
      ...
      <SectionCard title="Exposure Filters" subtitle="Narrow the model without changing the underlying source records.">
        ...
        <div className="dm-filters">
          ...
          <label htmlFor="political-money-search"><span>Committee search</span><input id="political-money-search" name="political_money_search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name or committee ID" /></label>

 

          <label htmlFor="political-money-cycle"><span>Election cycle</span><select id="political-money-cycle" name="political_money_cycle" value={cycle} onChange={(event) => setCycle(Number(event.target.value))}><option value={2026}>2026</option><option value={2024}>2024</option><option value={2022}>2022</option><option value={2020}>2020</option></select></label>

 

          <label htmlFor="political-money-state"><span>State</span><select id="political-money-state" name="political_money_state" value={state} onChange={(event) => setState(event.target.value)}>{STATES.map(([value, label]) => <option key={value || "all"} value={value}>{label}</option>)}</select></label>

 

          <label htmlFor="political-money-party"><span>Party</span><select id="political-money-party" name="political_money_party" value={party} onChange={(event) => setParty(event.target.value)}><option value="">All Parties</option><option value="DEM">Democratic</option><option value="REP">Republican</option><option value="IND">Independent</option></select></label>

 

          <label htmlFor="political-money-minimum-flow"><span>Minimum flow</span><input id="political-money-minimum-flow" name="political_money_minimum_flow" type="number" min="0" step="1000" value={minAmount} onChange={(event) => setMinAmount(event.target.value)} placeholder="0" /></label>
